...
from pipeline_context import ContextError, RUN_ROOT, resolve_patent_id, shown
...
def main() -> int:
    ...
    if not check:
        print(f"\nwrote {shown(OUT / 'structures-resolved.json')}"
              f" and {len(written)} drawings in {shown(svg_dir)}/")
    ...
    print(f"  and merge this into {shown(CURATED)}:\n")
    print(stub(missing))
    return 1
