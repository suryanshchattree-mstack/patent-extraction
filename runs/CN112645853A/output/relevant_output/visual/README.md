# Visual evidence for CN112645853A
...
- `name` (2 of 38): our structure was chosen by the compound name printed in the patent's TEXT near the drawing, and nothing about it came from the drawing. The two halves are independent and can genuinely disagree. This is the pairing that can catch a misread drawing.
- `structure` (36 of 38): the patent's words near the drawing name no compound we hold, so our record was found by matching structures. The halves then agree by construction. Such a comparison shows only that we hold the molecule at all, and it is labelled WEAK on the image itself.
...
67 of 79 panels are laid out the patent's way, and each panel records which template it used under
...
Where the pairing is by name, the drawing's SMILES and the gold's SMILES are both reduced to one canonical form and compared. That check ran on 2 structures and found 2 disagreements. The denominator is the point: zero out of zero would mean the check never ran.

It found 3 molecules that the patent draws and the gold holds no record of.

The 80 conflicts in `drawing-claims.json` are defects in the PATENT, not in the annotation. What each one asks the reviewer is whether we recorded what the patent really prints, contradiction and all, rather than quietly correcting it. Only 106 of them involve a drawing at all; the rest are two pieces of the patent's prose disagreeing, and those carry the page scan as their evidence rather than a picture that could not answer them.
...
and the vision pass captured it, on 20 of 20 pages here. The
...
```
python3 make_visual_evidence.py --patent-id CN112645853A
```
