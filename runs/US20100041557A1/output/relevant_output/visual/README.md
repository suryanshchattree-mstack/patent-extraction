# Visual evidence for US20100041557A1
...
The one disagreement, on p01, p02, p03, p04, p05, p06, p07, p08, p09, p10, p11, p12, p13, p14, p15, is benign and is left in
...
- `name` (0 of 3): our structure was chosen by the compound name printed in the patent's TEXT near the drawing, and nothing about it came from the drawing. The two halves are independent and can genuinely disagree. This is the pairing that can catch a misread drawing.
- `structure` (1 of 3): the patent's words near the drawing name no compound we hold, so our record was found by matching structures. The halves then agree by construction. Such a comparison shows only that we hold the molecule at all, and it is labelled WEAK on the image itself.
...
1 of 3 panels are laid out the patent's way, and each panel records which template it used under
...
It found 2 molecules that the patent draws and the gold holds no record of.

The 70 conflicts in `drawing-claims.json` are defects in the PATENT, not in the annotation. What each one asks the reviewer is whether we recorded what the patent really prints, contradiction and all, rather than quietly correcting it. Only 7 of them involve a drawing at all; the rest are two pieces of the patent's prose disagreeing, and those carry the page scan as their evidence rather than a picture that could not answer them.
...
and the vision pass captured it, on 15 of 15 pages here. The
...
```
python3 make_visual_evidence.py --patent-id US20100041557A1
```
