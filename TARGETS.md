...
| 8 | [`CN106008290A`](https://patents.google.com/patent/CN106008290A/en) | **Tejas** | blocked | 57098239 | CN | Intermediate Molecule | Method for preparing tembotrions <br>*selfcheck 35 pass, 1 warn, 0 fail on everything except the review census: 131 claims, 19.0 min against a pinned 15. This patent tells one two-step route five times, as Examples 1 to 5, so 108 census claims are the same comparisons asked once per telling. That is the open question the last paragraph of `ONE-ROUTE-TOLD-FOUR-TIMES.md` raises and leaves unowned, now measured on a patent where it breaches the budget. Pooling the tellings would fix it and is a review-queue change affecting every run, so I did not build it. Separately fixed on the way, with a measurement: `finalise.py` merge treated an all-null nested `quantity` as populated, so a section with no numbers overwrote one with them. 13 of 26 quantity-carrying records here were being lost, including tembotrione's own mass and yield, and 1 in the reference run, also tembotrione. See `runs/CN106008290A/NOTES.md`.*|
