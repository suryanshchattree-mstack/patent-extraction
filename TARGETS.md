...
| 7 | [`WO2024109718A1`](https://patents.google.com/patent/WO2024109718A1/en) | **Tejas** | blocked | 91195273 | WO | Exact Molecule | Method for preparing tembotrione, and intermediates <br>*45 pages, no text layer at all, so every character came from the vision pass. `selfcheck` 35 pass, 1 warn, 2 fail; both failures are the census, 150 claims and 21.8 min against the pinned 15. Fourth row in a row to hit that wall. **The finding worth reading:** the vision pass rendered 环磺酮 four ways and four of them said `sulcotrione`, which is 磺草酮, a different registered herbicide; corrected at the input with every original rendering recorded. Eight defects fixed in shared `pipeline/` code, one more found and left for an owner. Three of the eight were in the substance recall sweep, which was reporting substances the gold was holding: the name join was blind across languages, so a Chinese identifier and an English reading could never meet. See `runs/WO2024109718A1/NOTES.md`.*|
