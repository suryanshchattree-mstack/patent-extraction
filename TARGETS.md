...
| 8 | [`CN106008290A`](https://patents.google.com/patent/CN106008290A/en) | **Tejas** | blocked | 57098239 | CN | Intermediate Molecule | Method for preparing tembotrions <br>*selfcheck 35 pass, 1 warn, 2 fail. Both fails are the one census budget measure: 128 claims, 18.6 min against a pinned 15. Cause found and not patched, because the fix is in shared code and would change the read-only reference run: `finalise.py:208` treats an all-null nested `quantity` dict as populated, so a section with no numbers overwrites one with them, and five product masses and yields A1 read correctly are absent from the deliverable. The same 25 claims that cause the overrun trace to it. Pre-existing; the reference run shows the same shape. See `runs/CN106008290A/NOTES.md`.*|
